...
class CreateTenant extends Command
{
    ...
    protected $signature = 'tenant:create
                            ...
                            {--admin-phone= : Telefone do administrador}
     ...
     * @var string
     */
    protected $description = 'Criar um novo tenant com todas as configurações relacionadas';
    ...
    {
        $this->info('🚀 Criando novo tenant com todas as configurações...');
        $this->newLine();
        ...
        try {
            ...
            // Validar dados
            $this->validateData($tenantData, $adminData);
            ...
            $admin = $this->createAdminUser($tenant, $adminData);

            // Commit da transação
            DB::commit();

            // Exibir resumo
            $this->displaySummary($tenant, $admin, $tenantData, $adminData);
    ...
    /**
     * Obter dados do administrador via opções ou prompts
     */
    private function getAdminData(): array
    {
        return [
            'admin_name' => $this->option('admin-name') ?: $this->ask('Nome do administrador'),
            'admin_email' => $this->option('admin-email') ?: $this->ask('Email do administrador'),
            'admin_password' => $this->option('admin-password') ?: $this->secret('Senha do administrador'),
            'admin_phone' => $this->option('admin-phone') ?: $this->ask('Telefone do administrador', '(11) 99999-9999'),
        ];
    }
     ...
     * Validar dados
     */
    private function validateData(array $tenantData, array $adminData): void
    {
        $validator = Validator::make(array_merge($tenantData, $adminData), [
    ...
    private function createAdminUser(Tenant $tenant, array $data): TenantUser
    {
        $admin = TenantUser::create([
            ...
            'permissions' => [
                ...
                'manage_theme', 'manage_seo', 'manage_portal_settings'
            ],
        ]);
     ...
     */
    private function displaySummary(Tenant $tenant, TenantUser $admin, array $tenantData, array $adminData): void
    {
        $this->newLine();
        ...
        $this->newLine();
        $this->info('👤 USUÁRIO ADMINISTRADOR:');
        $this->newLine();
        ...
        $this->line("🔑 Senha: {$adminData['admin_password']}");
